...
func (r *DefaultTrafficRepository) CreateTraffic(traffic *domain.Traffic) error {
    // Конвертируем ExchangeConfig в JSON
    var exchangeConfigJSON string
    if traffic.ExchangeConfig != nil {
        data, err := json.Marshal(traffic.ExchangeConfig)
        if err != nil {
            return fmt.Errorf("failed to marshal exchange config: %w", err)
        }
        exchangeConfigJSON = string(data)
    }
    ...
    trafficModel := models.TrafficModel{
        ...
        Name:                 traffic.Name,
        ExchangeConfigJSON:   exchangeConfigJSON, // НОВОЕ
    }
...
func (r *DefaultTrafficRepository) UpdateTraffic(input *trafficdto.EditTrafficInput) error {
    ...
    // НОВОЕ: Обновляем конфигурацию курсов
    if input.ExchangeConfig != nil {
        exchangeConfigData, err := json.Marshal(input.ExchangeConfig)
        if err != nil {
            return fmt.Errorf("failed to marshal exchange config: %w", err)
        }
        updates["exchange_config_json"] = string(exchangeConfigData)
    }
...
// Добавляем метод для обновления только конфигурации курсов
func (r *DefaultTrafficRepository) UpdateExchangeConfig(trafficID string, config *domain.ExchangeConfig) error {
    if trafficID == "" {
        return fmt.Errorf("trafficID cannot be empty")
    }

    exchangeConfigData, err := json.Marshal(config)
    if err != nil {
        return fmt.Errorf("failed to marshal exchange config: %w", err)
    }
    ...
    result := r.DB.Model(&models.TrafficModel{}).
        ...
        Updates(map[string]interface{}{
            "exchange_config_json": string(exchangeConfigData),
            "updated_at":           time.Now(),
        })

    if result.Error != nil {
        return fmt.Errorf("failed to update exchange config: %w", result.Error)
    }
...
// Обновляем другие методы получения для использования маппера
func (r *DefaultTrafficRepository) GetTrafficRecords(page, limit int32) ([]*domain.Traffic, error) {
    ...
    for i, trafficModel := range trafficModels {
        trafficRecords[i] = r.mapTrafficModelToDomain(&trafficModel)
    }

    return trafficRecords, nil
}

// Обновляем методы получения трафика для включения конфигурации курсов
func (r *DefaultTrafficRepository) GetTrafficByID(trafficID string) (*domain.Traffic, error) {
    ...
    return r.mapTrafficModelToDomain(&trafficModel), nil
}

// Вспомогательный метод для маппинга
func (r *DefaultTrafficRepository) mapTrafficModelToDomain(trafficModel *models.TrafficModel) *domain.Traffic {
    ...
    // НОВОЕ: Парсим конфигурацию курсов
    if trafficModel.ExchangeConfigJSON != "" {
        var exchangeConfig domain.ExchangeConfig
        if err := json.Unmarshal([]byte(trafficModel.ExchangeConfigJSON), &exchangeConfig); err == nil {
            traffic.ExchangeConfig = &exchangeConfig
        } else {
            // Логируем ошибку, но не прерываем выполнение
            slog.Warn("Failed to parse exchange config JSON", 
                "trafficID", trafficModel.ID, 
                "error", err)
        }
    }
...
func (r *DefaultTrafficRepository) GetTrafficByTraderID(traderID string) ([]*domain.Traffic, error) {
    ...
    for _, tm := range trafficModels {
        traffics = append(traffics, r.mapTrafficModelToDomain(&tm))
    }
