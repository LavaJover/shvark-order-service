...
func (r *DefaultTrafficRepository) CreateTraffic(traffic *domain.Traffic) error {
	trafficModel := entities.TrafficModel{
		ID: uuid.New().String(),
...
func (r *DefaultTrafficRepository) DeleteTraffic(trafficID string) error {
	if err := r.DB.Delete(&entities.TrafficModel{ID: trafficID}).Error; err != nil {
		return err
	}
...
func (r *DefaultTrafficRepository) GetTrafficRecords(page, limit int32) ([]*domain.Traffic, error) {
	var trafficModels []entities.TrafficModel
	var total int64

	// Подсчёт числа записей
	r.DB.Model(&entities.TrafficModel{}).Count(&total)
...
func (r *DefaultTrafficRepository) GetTrafficByID(trafficID string) (*domain.Traffic, error) {
	var trafficModel entities.TrafficModel
...
func (r *DefaultTrafficRepository) DisableTraderTraffic(traderID string) error {
	err := r.DB.Model(&entities.TrafficModel{}).Where("trader_id = ?", traderID).Update("enabled", false).Error
	return err
}

func (r *DefaultTrafficRepository) EnableTraderTraffic(traderID string) error {
	err := r.DB.Model(&entities.TrafficModel{}).Where("trader_id = ?", traderID).Update("enabled", true).Error
	return err
}

func (r *DefaultTrafficRepository) GetTraderTrafficStatus(traderID string) (bool, error) {
	var count int64
	if err := r.DB.Model(&entities.TrafficModel{}).Where("trader_id = ? AND enabled = ?", traderID, true).Count(&count).Error; err != nil {
		return false, nil
	}
