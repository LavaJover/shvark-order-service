...
func (r *DefaultOrderRepository) CreateOrder(order *domain.Order) (string, error) {
	orderModel := entities.OrderModel{
		ID: uuid.New().String(),
...
func (r *DefaultOrderRepository) UpdateOrderStatus(orderID string, newStatus domain.OrderStatus) error {
	updatedOrderModel := entities.OrderModel{
		ID: orderID,
		Status: newStatus,
	}
...
func (r *DefaultOrderRepository) GetOrdersByTraderID(
	...
	var orderModels []entities.OrderModel
	var total int64
    ...
    baseQuery := r.DB.Model(&entities.OrderModel{}).
        Preload("BankDetail").
...
func (r *DefaultOrderRepository) GetCreatedOrdersByClientID(clientID string) ([]*domain.Order, error) {
	var orderModels []entities.OrderModel
	if err := r.DB.Model(&entities.OrderModel{}).Preload("BankDetail").Where("client_id = ? AND status = ?", clientID, domain.StatusCreated).Find(&orderModels).Error; err != nil {
		return nil, err
	}
